...
But Rework has no high level API and pays no attention to your CSS code style and 
indentations. So it can’t be used in text editor plugins.
...
set new value. As you can remember, PostCSS tries to save origin CSS byte-to-byte, 
when it's possible:
...
As you see, some at-rules don’t contain any children (like `@charset` or `@import`), 
some of at-rules can contain only declarations (like `@font-face` or `@page`), 
but most of them can contain rules and nested at-rules (like `@media`, `@keyframes` 
and others).

Parser select `AtRule` content type by its name. If you create `AtRule` node manually, 
it will detect own content type with new child type on first `append` or other add method call:
