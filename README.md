...
Using [source maps], a browser’s development tools can indicate
the original position of your code in the source files, before
any transformations were applied. For example, an inspector can show
the position of a rule in a Sass source file, even if that file
was compiled to CSS, concatenated, and minified.

To ensure that an accurate source map is produced, every CSS processing step
should update the map that was generated by the previous step. For example,
if a set of stylesheets are going to be compiled from Sass, concatenated,
then minified, the concatenation tool should update the map generated during
Sass compilation, and the minifier should update the map generated
by the concatenation tool.
...
* External: The source map lives outside the CSS in a separate file.
 ...
 /*# sourceMappingURL=main.out.css.map */
  ```
* Inline: The source map is embedded in the CSS file
  as a Base64-encoded comment.
...
PostCSS has great source map support. It can read and interpret maps
