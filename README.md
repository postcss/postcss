...
in a Sass source file, even if that file was compiled to CSS, concatenated,
and minified.
...
* `prev` {string|object|boolean}: source map content from a previous processing
  ...
  source map automatically (based on comments within the source CSS),
  but you can use this option to identify it manually. If desired, you can omit
  the previous map with `prev: false`.
