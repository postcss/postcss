...
class LazyResult {
  ...
  visitSync () {
    ...
    if (this.listeners.Exit) {
      for (let visitor of this.listeners.Exit) {
        let promise = visitor(this.helpers)
        if (isPromise(promise)) {
          throw useAsyncError()
        }
      }
