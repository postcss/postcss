...
class LazyResult {
  ...
  run (plugin) {
    ...
    if (fn) {
      try {
        return fn(this.result.root, this.result)
      } catch (error) {
        this.handleError(error, plugin)
        throw error
      }
    ...
    let add = (plugin, type) => {
      if (plugin[type]) {
        ...
        this.listeners[type].push((...args) => {
          this.result.lastPlugin = plugin
          return plugin[type](...args)
        })
      }
    }
    for (let plugin of this.result.processor.plugins) {
      if (typeof plugin === 'object') {
        for (let node of ['decl', 'rule', 'atrule', 'comment']) {
          add(plugin, node)
          add(plugin, node + 'Exit')
        }
        add(plugin, 'rootExit')
      }
    }
  ...
  visitSync () {
    ...
    let run = (node, phase) => {
      let visitors = this.listeners[node.type + phase]
      if (!visitors) return
      let proxy = node.toProxy()
      for (let visitor of visitors) {
        let promise
        try {
          promise = visitor(proxy, this.result)
        } catch (e) {
  ...
  async visitAsync () {
    ...
    let run = async (node, phase) => {
      ...
      for (let visitor of visitors) {
        try {
          await visitor(proxy, this.result)
        } catch (e) {
          this.handleError(e, this.result.lastPlugin)
          throw node.addToError(e)
        }
