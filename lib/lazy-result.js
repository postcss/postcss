  ...
  walkSync (node) {
    node.each(child => {
      ...
      if (child[isClean]) {
        ...
        child[isClean] = true
        let type = TYPE_TO_CLASS_NAME[child.type]
        let key = false
        if (child.type === 'decl') key = '-' + child.prop.toLowerCase()
        if (child.type === 'atrule') key = '-' + child.name.toLowerCase()

        if (this.listeners[type]) {
          this.visitSync(this.listeners[type], child.toProxy())
        }
        if (this.listeners[type + key]) {
          this.visitSync(this.listeners[type + key], child.toProxy())
        }
        if (child.nodes) {
          this.walkSync(child)
        }
        if (this.listeners[type + 'Exit']) {
          this.visitSync(this.listeners[type + 'Exit'], child.toProxy())
        }
        if (this.listeners[type + 'Exit' + key]) {
          this.visitSync(this.listeners[type + 'Exit' + key], child.toProxy())
        }
      }
    })
  }
