...
export default class CssSyntaxError extends SyntaxError {
    ...
    showSourceCode(color) {
        ...
        if ( typeof color === 'undefined' ) color = this.colorSupport();
        if ( color ) {
    ...
    colorSupport() {
        // Tests from https://github.com/chalk/supports-color
        if ( typeof process === 'undefined' )                    return false;
        if ( process.stdout && !process.stdout.isTTY )           return false;
        if ( process.platform === 'win32' )                      return true;
        if ( 'COLORTERM' in process.env )                        return true;
        if ( process.env.TERM === 'dumb' )                       return false;
        if ( /^(screen|xterm|vt100)/i.test(process.env.TERM) )   return true;
        if ( /color|ansi|cygwin|linux/i.test(process.env.TERM) ) return true;
        return false;
    }
