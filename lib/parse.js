...
class Parser {
    ...
    tokenize() {
        this.tokens = tokenize(this.source, this.opts, this.root.prevMap);
    }
    ...
    missedSemicolon(tokens, colon) {
        if ( this.opts.safe ) {
            ...
            this.error('Missed semicolon', token[3]);
        }
    }
    ...
    error(message, pos) {
        throw new CssSyntaxError(message, this.source, pos,
            this.opts.from, this.root.prevMap);
    }
