...
// CSS parser
class Parser {
    constructor(source, opts = { }, tokenize = { }) {
        this.source = source.toString();
        this.opts   = opts;
        if ( this.opts.from ) this.from = path.resolve(this.opts.from);

        this.tokens = tokenize(this.source, opts);
    ...
    decl(tokens) {
        ...
        for ( var i = tokens.length - 1; i > 0; i-- ) {
            ...
            if ( token[0] == 'word' && token[1] == '!important' ) {
                node.important = true;
                var string = this.map(tokens.splice(i, tokens.length - i),
                                   i => i[1] ).join('');
    ...
    raw(node, prop, tokens) {
        ...
        if ( !clean ) {
            var origin = this.map(tokens, i => i[1] ).join('');
...
module.exports = function (source, opts = { }) {
    ...
    var parser = new Parser(source, opts, tokenize);
    parser.setMap();
    parser.loop();
