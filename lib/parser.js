...
let tokenizer = require('@csstools/css-tokenizer').tokenizer;
...
class Parser {
  ...
  createTokenizer() {
    this.tokenizer = tokenizer(this.input, {
      ...
      onParseError: (err) => {
        switch (err.message) {
          case 'Unexpected EOF while consuming a comment.':
            throw this.input.error('Unclosed comment', err.sourceStart)
          case 'Unexpected newline while consuming a string token.':
            throw this.input.error('Unclosed string', err.sourceStart)

          default:
            throw this.input.error(err.message, { offset: err.sourceStart }, { offset: err.sourceEnd + 1 })
        }
      }
    })
    ...
    this.tokenizer.endOfFileWithBack = () => {
  ...
  parse() {
    let token
    while (!this.tokenizer.endOfFileWithBack()) {
      token = this.tokenizer.nextTokenWithBack()
  ...
  other(start) {
    ...
    while (token) {
      ...
      if (this.tokenizer.endOfFileWithBack()) {
        break;
      }
    ...
    if (this.tokenizer.endOfFileWithBack()) end = true
  ...
  decl(tokens, customProperty) {
    ...
    // TODO :
    // "!important" is not one token, but a delim and an ident.
    // This algorithm below needs to be re-written to support this.
    ...
    for (let i = tokens.length - 1; i >= 0; i--) {
      ...
      if (token[1].toLowerCase() === '!important') {
        ...
        for (let j = i; j > 0; j--) {
          let type = cache[j][0]
          if (str.trim().indexOf('!') === 0 && (type !== 'whitespace-token' && type !== 'comment')) {
            break
          }
          str = cache.pop()[1] + str
        }
        if (str.trim().indexOf('!') === 0) {
          node.important = true
          if (str !== ' !important') node.raws.important = str
          tokens = cache
        }
      }

      if (token[0] !== 'whitespace-token' && token[0] !== 'comment') {
        break
      }
    }
