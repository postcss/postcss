...
class Parser {
    ...
    word(start) {
        ...
        if ( this.input.safe ) {
            ...
            this.input.error('Unknown word', start[2]);
        }
    }
    ...
    decl(tokens) {
        ...
        if ( last[0] == ';' ) {
            this.setEnd(node, last[2]);
            this.semicolon  = true;
            tokens.pop();
        } else {
            this.setEnd(node, last[3]);
        }

        while ( tokens[0][0] != 'word' ) {
            node.before += tokens.shift()[1];
        }
        if ( node.source ) node.source.start = tokens[0][2];
    ...
    atrule(token) {
        ...
        if ( node.name === '' ) {
            if ( this.input.safe ) {
                ...
                this.input.error('At-rule without name', token[2]);
            }
        }
        this.init(node, token[2]);
        ...
        if ( params.length ) {
            ...
            if ( last ) {
                this.setEnd(node, params[ params.length - 1 ][3]);
        ...
        if ( this.current.parent ) {
            this.setEnd(this.current, token[2]);
            this.current = this.current.parent;
        } else if ( !this.input.safe ) {
            this.input.error('Unexpected }', token[2]);
        } else {
            this.current.after += '}';
        }
    ...
    endFile() {
        if ( this.current.parent && !this.input.safe ) {
            var start = this.current.source ? this.current.source.start : null;
            this.input.error('Unclosed block', start);
        }
    ...
    unknownWord(node, token) {
        if ( this.input.safe ) {
            if ( node.source ) node.source.start = token[2];
            node.before += node.prop + node.between;
            node.prop    = tokens.shift()[1];
            node.between = '';
        } else {
            this.input.error('Unknown word', token[2]);
        }
    }
    ...
    missedSemicolon(tokens, colon) {
        if ( this.input.safe ) {
            ...
            this.input.error('Missed semicolon', token[3]);
        }
    }
    ...
    init(node, start) {
        ...
        if ( this.input.withSource ) {
            node.source = { start: start, content: this.input.css };
    ...
    setEnd(node, pos) {
        if ( node.source ) node.source.end = pos;
    }
