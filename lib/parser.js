...
// Max number of tokens to have in the tokens array otherwise V8 treats it like a slower sparse array
// http://stackoverflow.com/questions/16961838/working-with-arrays-in-v8-performance-issue
var MAX_TOKEN_ARRAY_SIZE = 64000; 
...
class Parser {
    ...
    tokenize() {
        var tokens = tokenize(this.input);
        if (tokens.length > MAX_TOKEN_ARRAY_SIZE) {
          var tokenArrays = [];
          while (tokens.length > 0) {
            tokenArrays.push(tokens.splice(0, MAX_TOKEN_ARRAY_SIZE));
          }
          tokens = {
            tokenArrays: tokenArrays,
            push: function(elem) { return this.tokenArrays[this.tokenArrays.length - 1].push(elem); },
            pop: function() {
              var tokens = this.tokenArrays[this.tokenArrays.length - 1];
              return tokens ? tokens.pop() : tokens;
            },
            unshift: function(elem) { return this.tokenArrays[0].unshift(elem); },
            shift: function() {
              if (this.tokenArrays[0].length === 0) this.tokenArrays.shift();
              var tokens = this.tokenArrays[0];
              return tokens ? tokens.shift() : tokens;
            }
          };
        }
        this.tokens = tokens;
