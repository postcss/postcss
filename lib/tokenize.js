...
module.exports = function tokenizer(input, options = {}) {
  ...
  function checkIfThreeCodePointsWouldStartAnIdentSequence() {
    ...
    if (codePointSource[cursor] === HYPHEN_MINUS) {
      ...
      if (codePointSource[cursor + 1] === REVERSE_SOLIDUS && codePointSource[cursor + 2] !== LINE_FEED) {
        return true;
      }
  ...
  function checkIfThreeCodePointsWouldStartANumber() {
    ...
    } else if (isDigitCodePoint(codePointSource[cursor])) { // digit
      // Return true.
      return true;
    }
  ...
  function consumeNumber() {
    ...
    if (
      (codePointSource[cursor] === LATIN_SMALL_LETTER_E || codePointSource[cursor] === LATIN_CAPITAL_LETTER_E) &&
      isDigitCodePoint(codePointSource[cursor + 1])
    ) {
      // 5.1. Consume them.
      advanceCodePoint(2);
  ...
  function nextToken() {
    ...
    switch (peeked) {
      ...
      case REVERSE_SOLIDUS:
        ...
        if (!ignore) {
          throw input.error('Unclosed character escape', representationStart)
        }
